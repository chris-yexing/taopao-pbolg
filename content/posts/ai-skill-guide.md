...
如果你用过 Claude Code，一定见过这样的提示：`/init`、`/review`、`/security-review`。这些斜杠命令，就是 AI Skill——让 Claude 切换成"专家模式"的机制。

告诉 Claude 做什么，它临场发挥，这是普通对话。Skill 则是把专家的思维框架和执行步骤提前写好，一键激活。区别不是大不大，而是稳不稳——本文从原理到实战，带你把这个机制搞清楚。
...
**用户触发（User-invoked）**：主动输入 `/skill-name`，适合明确知道自己要什么时。

**模型自动触发（Model-invoked）**：Claude 根据对话内容判断场景，自动激活相关 Skill。比如你说"帮我看看这个 PR"，Claude 可能直接调用 `/review`，你不用显式输入命令。
...
用下来最大的感受是：Skill 改变的不是 Claude 的能力上限，而是它发挥能力的稳定性。内置 Skill 处理代码审查、安全扫描这些高频场景已经够用；自定义 Skill 则让你把任何可以语言描述的"专家视角"固化下来，张雪峰也好，特朗普也好，或者你自己团队特定的 review 标准。

关键是想清楚一件事：什么场景下你希望 Claude 每次都按同一套逻辑走，而不是靠它临场判断。那就是该写 Skill 的时候。
